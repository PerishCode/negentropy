...
if [ -n "${TARGET:-}" ]; then
  NEGENTROPY_BUILD_VERSION="$RELEASE_VERSION" cargo build --release --locked -p cli --target "$TARGET"
  BIN="$ROOT/target/$TARGET/release/$NAME"
else
  NEGENTROPY_BUILD_VERSION="$RELEASE_VERSION" cargo build --release --locked -p cli
